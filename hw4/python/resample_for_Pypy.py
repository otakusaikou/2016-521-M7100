...
import sys
from trans8param_for_Pypy import get8params
...
def img2array(imgObj):
    """Transform the PIL image object to numpy array."""
...
def main():
    ...
    # Save result
    if numBand == 1:    # Panchromatic image
        mode = 'L'
        resImg = resImg.reshape(height, width)
    else:
        mode = 'RGB'    # Multi-spectral image
        resImg = resImg.reshape(height, width, numBand)
    fname, ext = os.path.splitext(imageName)
    byt = resImg.astype(np.uint8).tobytes()

    Image.frombytes(mode, (width, height), byt).save(fname + "_modified" + ext)
